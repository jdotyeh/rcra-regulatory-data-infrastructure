...
setwd("/Users/junliangye/GitHub/rcra-regulatory-data-infrastructure")

# ── Load data ─────────────────────────────────────────────────────────────────

complete   <- read_csv("data/pipeline_rcra_downloads/PIPELINE_RCRA_00_COMPLETE.csv", # nolint
                       col_types = cols(REGISTRY_ID = col_character(),
                                        SOURCE_ID   = col_character(),
                                        .default    = col_character()))

facilities <- read_csv("data/rcra_downloads/RCRA_FACILITIES.csv",
                       col_types = cols(ID_NUMBER = col_character(),
                                        .default  = col_character()))

viosnc     <- read_csv("data/rcra_downloads/RCRA_VIOSNC_HISTORY.csv",
                       col_types = cols(ID_NUMBER = col_character(),
